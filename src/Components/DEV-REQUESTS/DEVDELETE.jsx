...
// Creating a functional component called DEVDELETE
function DEVDELETE() {
  ...
  const deleteUser = id => {
    fetch(`https://jsonplaceholder.typicode.com/users/${id}`, {
      method: "DELETE", // Setting HTTP method to DELETE
    })
      .then(response => response.json()) // Parsing the response as JSON
      .then(() => {
        setUsers(values => { // Removing the deleted user from the users state
          return values.filter(item => item.id !== id);
        });
        AppToaster.show({ // Showing notification for successful user deletion
...
export default DEVDELETE; // Exporting the DEVDELETE component as default
